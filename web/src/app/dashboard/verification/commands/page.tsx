...
const DEFAULT_SHEERID_COMMANDS = [
  {
    command_name: "/start",
    description: "Menu utama",
    default_text:
      "🔐 SheerID Verification Bot\n\nSelamat datang! Bot ini membantu verifikasi akun student/teacher.\n\nGunakan /verify untuk memulai verifikasi.",
  },
  {
    command_name: "/verify",
    description: "Mulai verifikasi",
    default_text:
      "📝 Pilih Jenis Verifikasi\n\nSilakan pilih layanan yang ingin diverifikasi:",
  },
  {
    command_name: "/balance",
    description: "Cek saldo poin",
    default_text: "💰 Saldo Anda\n\n{balance} poin",
  },
  {
    command_name: "/history",
    description: "Riwayat verifikasi",
    default_text: "📋 Riwayat Verifikasi\n\nBerikut riwayat verifikasi Anda:",
  },
  {
    command_name: "/help",
    description: "Bantuan",
    default_text:
      "❓ Bantuan\n\n/start - Menu utama\n/verify - Mulai verifikasi\n/balance - Cek saldo\n/history - Riwayat\n/help - Bantuan",
  },
];

export default function SheerIDCommandsPage() {
  ...
  const fetchBots = async () => {
    ...
    if (result.data?.bots) {
      const sheeridBots = result.data.bots.filter(
        (b: any) => b.bot_type === "sheerid",
      );
      setBots(sheeridBots);
      if (sheeridBots.length > 0) {
        setSelectedBot(sheeridBots[0].id);
      }
    }
    setIsLoading(false);
  };
  ...
  const getCommandText = (commandName: string): string => {
    ...
    const defaultCmd = DEFAULT_SHEERID_COMMANDS.find(
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold">⚙️ SheerID Commands</h1>
          <p className="text-[var(--text-muted)]">
            Atur respons bot verifikasi
          </p>
        </div>
        {bots.length > 0 && (
          <select
      ...
      <div className="glass-card p-4 border-l-4 border-green-500">
        <p className="text-sm">
          💡 <strong>Tips:</strong> Gunakan{" "}
          <code className="bg-[var(--bg-dark)] px-1 rounded">
            {"{balance}"}
          </code>{" "}
          untuk menampilkan saldo poin user. Pesan yang diatur di sini otomatis
          digunakan oleh bot Telegram.
        </p>
      </div>
      ...
      {bots.length === 0 ? (
        <div className="glass-card p-10 text-center">
          <p className="text-[var(--text-muted)]">
            Belum ada SheerID Bot. Tambahkan bot terlebih dahulu.
          </p>
        </div>
      ) : isLoading ? (
        ...
        <div className="space-y-4">
          {DEFAULT_SHEERID_COMMANDS.map((cmd) => (
              ...
              {editingCommand === cmd.command_name ? (
                ...
                <div>
                  ...
                  <button
                    ...
                    className="mt-3 px-4 py-2 text-sm rounded-lg bg-green-500/20 text-green-400 hover:bg-green-500/30"
                  >
                    ✏️ Edit Response
                  </button>
                </div>
